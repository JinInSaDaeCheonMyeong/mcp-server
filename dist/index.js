...
// 공고 검색 함수
async function searchNotices(query, limit = 5) {
    try {
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: "search_notices",
                description: "사용자의 요청과 관련된 스타트업 공고를 검색합니다. " +
                    "키워드나 조건을 포함한 자연스러운 쿼리를 입력하면 " +
                    "유사한 공고들을 찾아 반환합니다. " +
                    "예: '서울에서 열리는 공고', '기술 스타트업 지원', '멘토링 프로그램'",
                inputSchema: zodToJsonSchema(SearchNoticeArgsSchema),
            },
        ],
    };
});
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        if (name === "search_notices") {
            ...
            const results = await searchNotices(parsed.data.query, parsed.data.limit);
